...
metmap_combined %>%
  ...
  dplyr::summarize(z_threshold = mean(penetrance) + sd(penetrance)) %>% 
...
metmap_combined %>%
  ...
  dplyr::mutate(
    ...
    tropism = ifelse(penetrance_z >= z_threshold, "high-tropic", "tropic"),
    tropism = ifelse(penetrance == 0, "non-tropic", tropism),
    organ_tropism = paste0(organ, "_", tropism)) %>%
  dplyr::select(contains("name"), contains("id"), contains("CI."),
                contains("penetrance"), z_threshold, everything()) %>%
