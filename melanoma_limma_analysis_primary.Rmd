...
# set project name
project_name <- "depmap_melanoma_limma_primary"
...
For more about how the Depmap data was originally processed, please refer to the  [original Depmap paper](https://pmc.ncbi.nlm.nih.gov/articles/PMC5667678/).
...
contrast.matrix <- makeContrasts(
  ...
  primary_vs_skin                      = skin - primary,
  primary_vs_lung                      = lung - primary,
  primary_vs_liver                     = liver - primary,
  primary_vs_lymph_node                = lymph_node - primary,
  primary_vs_pleural_effusion          = pleural_effusion - primary,
  primary_vs_central_nervous_system    = central_nervous_system - primary,
  primary_vs_ascites                   = ascites - primary,
  primary_vs_small_intestine           = small_intestine - primary,
  ## skin as normative
...
topTable(fit2, coef = "primary_vs_skin", adjust.method = "BH",
...
topTable(fit2, coef = "primary_vs_lung", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>% 
  dplyr::left_join(human_biomart, by = "gene_name") %>%
  dplyr::left_join(
...
topTable(fit2, coef = "primary_vs_liver", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>% 
  dplyr::left_join(human_biomart, by = "gene_name") %>%
  dplyr::left_join(
...
topTable(fit2, coef = "primary_vs_lymph_node", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>% 
  dplyr::left_join(human_biomart, by = "gene_name") %>%
  dplyr::left_join(
...
topTable(fit2, coef = "primary_vs_pleural_effusion", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>% 
  dplyr::left_join(human_biomart, by = "gene_name") %>% 
  dplyr::left_join(
...
topTable(fit2, coef = "primary_vs_central_nervous_system", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>% 
  dplyr::left_join(human_biomart, by = "gene_name") %>%
  dplyr::left_join(
...
topTable(fit2, coef = "primary_vs_ascites", adjust.method = "BH",
         number = nrow(cts_filtered)) %>%
  tibble::rownames_to_column(var = "gene_name") %>%
  dplyr::left_join(human_biomart, by = "gene_name") %>%
  dplyr::left_join(
...
```{r warning=FALSE, message=FALSE}
## make list
de_list <- list(skin_de, lung_de, liver_de, lymph_de, pe_de, cns_de, ascite_de, si_de)

## name list elements
names(de_list) <- c(
  "primary_vs_skin_degs",
  "primary_vs_lung_degs",
  "primary_vs_liver_degs",
  "primary_vs_lymph_node_degs",
  "primary_vs_pleuraleffusion_degs",
  "primary_vs_cns_degs",
  "primary_vs_ascite_degs",
  "primary_vs_smallintestine_degs")
